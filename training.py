...
def optimize_q(
    ...
    if train_state.config.relabel_reward_with_model:
        # No-Gate baseline: bootstrap Q on the (frozen) reward model R_psi(s, a)
        # instead of the observed proxy reward.
        action_batch_one_hot = torch.nn.functional.one_hot(
            action_batch, int(train_state.env.action_space.n))[:, 0, :].float()
        with torch.no_grad():
            if train_state.config.use_next_state_for_reward:
                reward_batch = train_state.reward_network(
                    state_batch, action_batch_one_hot, next_state_batch).squeeze(1).detach()
            else:
                reward_batch = train_state.reward_network(
                    state_batch, action_batch_one_hot).squeeze(1).detach()
...
def update_networks(
    ...
    if live_update and not train_state.config.freeze_reward_model:
