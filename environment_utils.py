...
from pycolab import ascii_art as _pycolab_ascii_art

from BoxMovingEnv import BoxMovingEnv

Palette.__deepcopy__ = lambda sel, mem: sel  # monkey patch the deepcopy


def _ascii_art_to_uint8_nparray(art):  # numpy>=2 compat: vstack needs a sequence, not a generator
    error_text = (
    ...
    try:
        arr = np.vstack([np.frombuffer(line.encode('ascii'), dtype=np.uint8) for line in art])
    except (ValueError, UnicodeEncodeError) as e:
        raise ValueError('{} (original error: {})'.format(error_text, e))
...
_pycolab_ascii_art.ascii_art_to_uint8_nparray = _ascii_art_to_uint8_nparray
...
def set_deployment(env, deployment):
    try:
        ...
        pass
    if isinstance(env, BoxMovingEnv):
...
def make_env(env_class):
    ...
    if is_safety_env:
        ...
        return env
    elif env_class == BoxMovingEnv:
        return RewardsWrapper(BoxMovingEnv(), scale=0.2)
    return gym.make(name)
    

def get_default_config(env_class):
    config = Config()
    if env_class == BoxMovingEnv:
