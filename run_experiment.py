...
def run_experiments(name: str, cfg: Config, parallel: bool = False, runner=run_experiment):
    ...
    if not parallel:
        for seed in cfg.seeds:
            runner(seed, name, cfg)
        return
    multiprocessing.set_start_method('spawn')
    pool = multiprocessing.Pool(processes=len(cfg.seeds))
    pool.map(functools.partial(runner, name=name, cfg=cfg), cfg.seeds)
    print(f'Done running experiment {name}')

if __name__ == '__main__':
    # Main experiment: Modification-Considering Value Learning (MCVL) on the
    # gridworld environments. For each seed, run_experiment() trains an initial
    # (pre-deployment) agent, then runs two deployment phases on the same
    # pretrained state: one where tampering is possible without the gate, and one
    # protected by the MCVL forecast-and-score gate. Results for all phases are
    # written to results/<name>_<EnvClass>/ and consumed by plots.ipynb.
    #
    # Swap env_class to run on a different gridworld, e.g.:
    #   AbsentSupervisorEnvironment, RocksDiamondsEnvironment, TomatoWateringEnvironment
    env_class = TomatoWateringEnvironment
    config = get_default_config(env_class)
    run_experiments('mcvl', config, parallel=True)
    print('Done')
