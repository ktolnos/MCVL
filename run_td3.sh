#!/bin/bash
# Launch a single TD3 continuous-control run (td3.py) for one seed/config.
#
# The SBATCH directives below are provided as an example for Slurm clusters;
# they are ignored when the script is run directly with `bash run_td3.sh ...`.
...
ORACLE_REWARD=True
RELABEL_REWARD_WITH_MODEL=False
FROZEN_POLICY=False
USING_POLICY_STARTS=""
TAMPERING_THRESHOLD=""
OUTPUT_DIR="runs"
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --relabel_reward_with_model=*) RELABEL_REWARD_WITH_MODEL="${1#*=}" ;;
        --relabel_reward_with_model) RELABEL_REWARD_WITH_MODEL="$2"; shift ;;
...
if [ -z "$RUN_NAME" ]; then
    if [ -n "$LAST_COMMIT_MESSAGE" ]; then
        ...
        COMMIT_MSG=$(git log -1 --pretty=%s 2>/dev/null || echo run)
    fi
...
echo "RELABEL_REWARD_WITH_MODEL: $RELABEL_REWARD_WITH_MODEL"
...
python td3.py \
    ...
    --relabel_reward_with_model $RELABEL_REWARD_WITH_MODEL \
