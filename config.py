...
class Config:
    ...
    # "No-Gate" / "Reward-Model RL" baseline: bootstrap Q on the (frozen) reward
    # model R_psi instead of the observed proxy reward, with no forecast-and-score gate.
    relabel_reward_with_model: bool = False  # bootstrap Q on R_psi(s, a) instead of the observed reward
    freeze_reward_model: bool = False  # stop updating R_psi (the reward model, not the transition model P_hat)
